    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600">Build a new event and send it for approval.</p>
            </div>
                ...
                <form method="POST" action="{{ route('organizer.events.store') }}" class="space-y-6">
                    ...
                    <div class="grid gap-6">
                        <div>
                            ...
                            <input name="title" value="{{ old('title') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                        ...
                        <div>
                            ...
                            <select name="venue_id" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3">
                                ...
                                @foreach($venues as $venue)
                                    <option value="{{ $venue->id }}" @selected(old('venue_id') == $venue->id)>{{ $venue->name }} — {{ $venue->city }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <input type="date" name="date" value="{{ old('date') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Time</label>
                                <input type="time" name="time" value="{{ old('time') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Min age</label>
                                <input type="number" name="min_age" value="{{ old('min_age') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                            </div>
                        </div>
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Max participants</label>
                                <input type="number" name="max_participants" value="{{ old('max_participants') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                            </div>
                            <div>
                                ...
                                <select name="access_type" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3">
                                    ...
                                    <option value="presale" @selected(old('access_type') === 'presale')>Presale</option>
                                    <option value="waiting_list" @selected(old('access_type') === 'waiting_list')>Waiting List</option>
                                    <option value="open" @selected(old('access_type') === 'open')>Open Access</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Presale price</label>
                                <input type="number" step="0.01" name="presale_price" value="{{ old('presale_price') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Presale quantity</label>
                            <input type="number" name="presale_quantity" value="{{ old('presale_quantity') }}" class="mt-2 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white p-3" />
                        </div>
                    </div>
                    <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-red-600 px-5 py-3 text-white hover:bg-red-500">Submit event</button>
                </form>
            </div>
        </div>
    </div>
